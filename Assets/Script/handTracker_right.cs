...
public class handTracker_right : MonoBehaviour
{
    public float sphereSize = 0.01f;
    public Material jointMaterial;
    public Material palmMaterial;
    ...
    void Start()
    {
        foreach (TrackedHandJoint joint in System.Enum.GetValues(typeof(TrackedHandJoint)))
        {
            GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            sphere.transform.localScale = Vector3.one * sphereSize;
            sphere.name = "Right_" + joint.ToString();
    ...
    void Update()
    {
        ...
        {
            if (HandJointUtils.TryGetJointPose(joint, Handedness.Right, out MixedRealityPose jointPose))
            {
        ...
        if (targetObject != null)
        {
            ...
            Quaternion rotZ = Quaternion.AngleAxis(0, targetObject.transform.forward);
            ...
            foreach (var kvp in relativePositions)
            {
                ...
                if (jointSpheres.TryGetValue(joint, out GameObject sphere))
                {
                    ...
                    {
                        case TrackedHandJoint.ThumbTip:
                            rightThumb.transform.position = newWorldPos;
                            rightThumb.transform.rotation = finalPalmRotation;

                            break;
                        case TrackedHandJoint.IndexTip:
                            rightIndex.transform.position = newWorldPos;
                            rightIndex.transform.rotation = finalPalmRotation;
                            break;
                        case TrackedHandJoint.MiddleTip:
                            rightMid.transform.position = newWorldPos;
                            rightMid.transform.rotation = finalPalmRotation;
                            break;
                        case TrackedHandJoint.PinkyTip:
                            rightRing.transform.position = newWorldPos;
                            rightRing.transform.rotation = finalPalmRotation;
                            break;
                    }
                }
            }
